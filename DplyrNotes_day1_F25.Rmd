...
- piping our code using `%>%` (or `|>`)

This is the wordy version of the slides. 
...
If you have another package using a function called `select()` when you load these packages (which contain a function with that name), this is letting you know that R will use the more recently loaded version by default. 

If you want to use the other version, you can specify it by using `package_name::function_name()` when you call it. 
...
A tibble is just a more modern version of a data frame that prints more neatly.

To quote **R for DataScience**: A `tibble`, or `tbl_df`, is a modern reimagining of the data.frame, keeping what time has proven to be effective, and throwing out what is not. Tibbles are data.frames that are lazy and surly: they do less (i.e. they don’t change variable names or types, and don’t do partial matching) and complain more (e.g. when a variable does not exist). This forces you to confront problems earlier, typically leading to cleaner, more expressive code. Tibbles also have an enhanced `print()` method which makes them easier to use with large datasets containing complex objects.
...
dim(gapminder)
nrow(gapminder)
summary(gapminder) # this give you a six number summary of numeric variables, counts the number of times each category shows up for factor variables and... does what with string variables?

## lets change a variable to find out. 
my_gap <- gapminder # make my own copy
my_gap$country <- as.character(my_gap$country) # change country to a string variable instead of
summary(my_gap) # what do you see in the summary for country now?
```
...
We can use another package called `ggplot2` to make lovely figures. The `continent` variable is easily mapped to what is called a `facet` or to colors and a legend. 

*We will be covering ggplot2 soon, so for now, just copy/paste if you want to play with them, and sit back and enjoy these plots for now* 
...
Another type of plot to show how much easier ggplot2 will be... 
...
R has been around forever. It and it's predecessor, S have been the staple of statisticians. But, the push towards including data science has created a relatively large boom in new packages and techniques being available. 
...
```{r}
filter(gapminder, country == "Bulgaria", year > 1952)
```
...
Why `magrittr`? Because of this painting by Magritte!
...
"sends the previous result to the first argument of the next function"
...
To type the pipe symbol quickly in RStudio, use `cmd-shift-M` or `control-shift-M`.
...
What does that look like in penguin data? Try it.
...
gapminder %>% select(starts_with("bill"))
